...
Une fois l'apprentissage terminé, quand le service identifie un fruit dans une image, il retourne le discriminant "fruit" dans un tableau contenant les classes "poires", "pommes" et "bananes", avec leurs scores de confiance respectifs.
...
![Discriminant fruits, après un réapprentissage, comportant une nouvelle classe positive pour oranges et une classe négative pour cheese](images/retrain.png)
...
- Limites appliquées aux méthodes de détection des visages :
    - La taille maximale d'une image est de 2 Mo.
    - La taille maximale d'un fichier .zip est de 5 Mo, avec un maximum de 15 images.
...
- Incluez au moins 50 images positives par classe avant d'accéder à vos résultats d'apprentissage.
    ...
    - Le bénéfice d'un apprentissage appliqué à un discriminant sur un plus grand nombre d'images se stabilise autour de 5000 images. Même si vous pouvez dépasser les 5000 images, la précision risque de ne pas augmenter de façon significative alors que le temps de traitement sera encore plus long.
...
- Rognez ou redimensionnez vos images pour qu'elles atteignent une taille de 224 x 224 pixels. Le service est optimisé pour l'instant pour cette taille, même si un changement est possible plus tard.
    - Rognez l'image si son rapport hauteur/largeur est supérieur à 2:1 ou inférieur à 1:2.
    - Envisagez une découpe de l'image en plusieurs images carrées différentes, ou n'incluez que le centre de l'image, en fonction de ce qui est le plus important pour votre utilisation.
...
- Bien que le service soit capable de lire les étiquettes EXIF et de faire pivoter les images, pour optimiser la capacité de traitement, n'envoyez via le service que les images qui n'ont pas besoin de pivoter (l'étique EXIF **Orientation** est définie à `1`).
