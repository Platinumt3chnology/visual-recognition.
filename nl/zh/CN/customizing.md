...
对图像进行分类，并使用[创建定制分类器教程](/docs/services/visual-recognition/tutorial-custom-classifier.html)中的示例数据创建、培训和查询定制分类器之后，可以对自己的数据进行分类或创建自己的定制分类器。
{: shortdesc}
...
定制分类器是一组相互进行培训的类。这样您就能创建多构面分类器，这种分类器可以识别高度专业化的主题，同时还能提供每个单独类的分数。

在培训期间，为每个类上传正面示例的单独压缩 (.zip) 文件时，会创建类。例如，要创建名为“水果”的分类器，那么可在单个培训调用中，上传梨图像的 .zip 文件、苹果图像的 .zip 文件和香蕉图像的 .zip 文件。

还可以在同一培训调用中提供负面示例的 .zip 文件，以进一步优化分类器。负面示例文件不用于创建类。对于定制分类器“水果”，可以提供具有各种蔬菜图像的 .zip 文件。

![具有正面和负面示例的“水果”分类器](images/classifier.png)

培训完成后，服务在识别图像中的水果时，会将“水果”分类器作为数组返回，数组中包含“梨”、“苹果”和“香蕉”类及其各自的置信度分数。
...
![“水果”分类器使用新的正面类“橙子”和负面类“奶酪”进行了重新培训](images/retrain.png)

### 重新培训工作方式

如果用三组正面类图片（苹果、香蕉和梨）培训了一个分类器，那么系统会在内部培训三个模型。对于“苹果”模型，“苹果”中的图片组会培训为正面示例，“香蕉”和“梨”中上传的图片组会培训为负面示例。这样，系统就知道香蕉和梨与苹果不同。其他类也可用作“香蕉”和“梨”模型的负面示例。

接下来，假设您想用新的正面类 YellowPears 和 GreenPears 来重新培训分类器。为此，您需要手动查看旧的 pears.zip 文件夹，并将图像拆分成两个新文件夹：YellowPears.zip 和 GreenPears.zip。

**重要信息：**可以通过重新培训来拆分类定义，但这要求在组织数据时格外小心。您必须将**完全**相同的图像文件提交到原始培训期间使用的新文件夹（不能调整大小或者修改任何内容）。例如，在创建 YellowPears 或 GreenPears 时，应该将原始 pears.zip 培训集内的每个黄色梨图像原封不动地复制到 YellowPears.zip 文件夹中；而并非原封不动复制的所有图像都会位于“梨”培训集内，并且在培训 YellowPears 时被用作负面示例。

现在，您只需将 YellowPears.zip 和 GreenPears.zip 用作正面示例对系统进行重新培训即可。执行此操作时，系统会从原始 pears.zip 文件夹的 YellowPears 和 GreenPears 文件夹中识别到完全相同的图像，然后将这些图像作为新类的正面示例进行重新培训。规则是：如果在类的负面和正面集内同时找到完全一样的图像，那么将该图像保存在正面集内。
...
- 人脸检测方法的限制：
    - 最大图像大小为 2 MB。
    - 最大 .zip 文件大小为 5 MB，最多包含 15 个图像。

<!-- - The `POST /v3/recognize_text` method accept a maximum of 10 images per batch. -->

## 良好培训准则

以下准则并不是 API 强制实施的。但是，如果培训数据符合这些准则，服务的性能往往会更好：
...
- 在评估培训结果之前，每个类至少包含 50 个正面图像。
    - 假设培训数据具有相似的质量和内容，那么通常培训图像越多，结果准确性越高。
    - 如果每个 .zip 文件有 150-200 个图像，那么处理时间和准确性之间能达到最佳平衡。超过 200 个图像可提高准确性，但因为用时更长，所以收效递减。
    - 使用更多图像对分类器进行培训的优势大约在 5000 个图像时达到停滞期。虽然可以培训 5000 个以上的图像，但达到这一数量后不会使准确性显著提高，而只会增加处理时间。
- 包括负面类有助于改善结果。
    - 包含数量大致与正面图像相同的负面图像。正面和负面图像数量不同可能会降低所培训分类器的质量。
- 确保培训图像中的背景与您期望分类的背景相似。分类器的准确性可能会受到提供用于培训的图像种类的影响。
    - 例如，如果在培训“老虎”分类器时，培训的图像仅为在动物园中用手机拍摄的老虎图像，但分析的是专业摄影师拍摄的野外老虎图像，那么分类器的准确性会下降。
- 确保分类器主题事物的大小至少占图像总大小的 1/3。


有关培训的更多信息，请参阅 [Best practices for custom classifiers ![外部链接图标](../../icons/launch-glyph.svg "外部链接图标")](https://www.ibm.com/blogs/bluemix/2016/10/watson-visual-recognition-training-best-practices/){: new_window}。
...
- 定制分类器使用二进制“一对其他”模型针对其他类来培训每个类。系统假定一个分类器中的两个类不能同时出现，所以您应该创建不同的分类器来测试可以一起存在的类，如 `blue` 和 `sky`。或者，对于两个类同时存在的情况，可以创建单独的分类器，并测试像 `blueSky` 这样的类。

### 示例

假设您要使用网络摄像头监视分配的停车位。您培训了一个定制分类器，用于识别是自己的车停在该停车位、其他车停于该停车位、该停车位为空还是摄像头被阻挡。您为每种情况收集了培训示例，然后使用四个类培训了一个定制分类器。您的应用程序会对来自网络摄像头的图像进行分类，以报告停车位的状态，如果状态异常，系统会使用一条消息通知您。每次服务对来自摄像头的图像分类时，都会生成四个分数：`myCar`、`unknownCar`、`emptySpot` 和 `blockedCamera`。
...
- **分数意味着什么？**
    ...
    - 服务会试图对分数输出进行规范化，所以 0.5 是一个很好的决策阈值。缺省情况下，低于 0.5 的分数不会在 `/classify` 结果中进行报告。您可以通过设置 `/classify` 方法的阈值参数来覆盖此行为。这种规范化只基于培训数据进行计算，所以对于新数据或不同的应用程序上下文，您可能会发现使用其他阈值效果更好。
